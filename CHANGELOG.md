...
- **Empire audit: scrap-processing throughput** — the Raw resource supply section gains a **Scrap processing** card for every station with a processing module, modded ones included: how much feedstock actually arrives per hour against what the modules could eat, as a coverage bar — one live station reads 8,374 of 9,000 Raw Scrap/h — plus how many salvage ships are assigned, how much scrap is drifting in that sector, and whether the station makes the energy cells its processors need. Utilization is measured on the intake side only, because the game records nothing about a processing module's output.
