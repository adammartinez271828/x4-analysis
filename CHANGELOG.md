...
- **Build advisor: a "Modules" column replaces the score.** The Score column and its five weight sliders are gone. Every factor behind the score was normalized *per ware*, so it only ever compared sectors for the same ware, but the table shipped unfiltered and sorted by it, so rows with a **negative** shortfall ranked near the top. Rows now carry **Modules** instead: the nearby shortfall divided by what one production module of that ware makes per hour, signed. +20.2 means the gap would keep twenty modules busy; −0.1 means the neighbourhood is already slightly oversupplied. The table opens sorted by **Untapped Cr/h**, and the **estimated actual flows** box now starts checked. The yield is the ware's default recipe, not your faction's build method or a closed-loop variant, so read Modules as the size of the gap rather than as a build order.
- **Trade → Routing: how long from here to there, for any ship and any two stations.** A new sub-tab beside Trade Opportunities. Pick a ship and an origin and every station in the save is listed with gate **jumps**, the real one-way **route length** in km (measured through actual station and gate positions, not hop-counted) and the estimated **voyage time**, sorted nearest-first. Every hull is pickable, not just the traders. No prices and no cargo: it shares Trade Opportunities' route graph and time model exactly, so the two panes can never disagree about how long a run takes. The page spells out what the model does not know. Gate transits count as zero time, one-way gates are treated as two-way, and a free-flying ship is placed at its sector centre.
- **Empire → Standings: your standing with every faction, over time.** A step line per faction under the standings table, drawn from the game log's own reputation entries: each one records the exact −30..+30 rank *after* the change, so the line is what the game showed you at that moment. The subtitle says how far back the history reaches. The logbook is a rolling window, so early history exists only where the analyzer has already imported it (`seed-trends` over your archived saves fills it in), and gaps are left as gaps rather than drawn through. There is deliberately no "which activity earned me this" breakdown: the game omits that number from most entries.
- **Empire audit: scrap-processing throughput.** The Raw resource supply section gains a **Scrap processing** card for every station with a processing module, modded ones included: how much feedstock actually arrives per hour against what the modules could eat, as a coverage bar. One live station reads 8,374 of 9,000 Raw Scrap/h. The card also shows how many salvage ships are assigned, how much scrap is drifting in that sector, and whether the station makes the energy cells its processors need. Utilization is measured on the intake side only, because the game records nothing about a processing module's output.

### Fixes

- **Objects in the map's zones are finally where the game puts them.** Reported by a user who found an Erlking data vault nowhere near the spot `x4-analyzer find` sent them to. Almost everything in a sector sits inside a *zone*, and a savegame never writes down where a fixed zone is; it leaves that in the game's own map files. The analyzer read the missing value as "the middle of the sector", so every station, vault, derelict and wormhole inside one was drawn 100–190 km off for the Erlking vaults, and up to 675 km off for a station on a live save. Zone offsets now come straight from the installed game (a new `zones.csv`, regenerated by `extract-gamedata`), so the sector map, the routing distances and the Build Advisor's trader counts all measure between real positions. Nothing is cached: the next analysis of any save fixes itself. A zone the reference data has never heard of still falls back to the sector centre, and now says so once per run instead of quietly misplacing things.
...
- **Empire audit: "starving for inputs" now knows what the station makes itself.** Cover was measured as stock ÷ gross consumption, so an input the station produces on site (energy cells at a solar-panel station, hull parts at a self-supplying wharf) was permanently listed as STALLED: it holds no stock because it is consumed as fast as it is made. Rows now show **Produces/h** beside Consumes/h and a **Net status** measured against *net* demand, self-supplied inputs are called out as such (worded carefully, since the module producing them can be starved in turn), and a new checkbox (off by default) hides them. Wares the station never stores at all, such as scrap delivered straight into the processor, are no longer listed.
