...
- **`extract-gamedata` no longer dies on a mod's empty file** — reported by a user running about 40 mods: one mod ships a zero-byte macro file, and the XML reader raises "Document is empty" even in its most forgiving mode, so a single unreadable file out of thousands killed the entire extraction. Missing, empty and unparseable files are now all handled the same way wherever game data is read — skipped with a warning that names the file, so the extraction finishes and you can see exactly which file was ignored. The guard covers module, weapon, shield, engine and ship-modification extraction plus the text database, including two places that never checked at all.
