...
- Market tab data semantics (all reverse-engineered, validated against this save):
  ...
  - Build demand = `<insufficient>` wares under `<build><resources>`, but ONLY for builds of type ""/"build" (station construction). `type="buildship"` entries are queued wharf ship orders whose amounts are a wharf-wide aggregate repeated per order/ware — summing them inflated ship-material demand ~10x. Stations also repeat the same missing amount at station level and per buildprocessor: dedupe with max per (host, ware). New-station constructions sit on **free-floating build storages with no station ancestor** — don't require an object ancestor when collecting (this bug once zeroed claytronics demand).
  - Understocked = buyers (open `<trade buyer= amount=>` offers under `<offers>`, plus build hosts) holding < 25% of target level (stock + wanted).
