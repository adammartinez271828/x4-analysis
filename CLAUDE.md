...
- **Keep reference / models / experiments in sync — every time.** Learning something new about game behaviour is not done until all three agree. On any change to a reverse-engineered claim:
  ...
  `tests/test_experiments_register.py` enforces the mechanical half — unique ids, closed status and field vocabulary, citations that resolve, SUPERSEDED entries naming a live replacement, and a summary table that matches its own entries (it prints the correct rows on failure). It cannot check that a status is *true* or that `reference/` agrees with the register; that half is yours.
...
- **Build method**: what a station builds ships/drones/deployables/modules from is a *recipe variant* (`recipes.csv` `method`), resolved station override (`build_method` table) → owner faction rule (`faction_meta.build_method`) → race default, with a per-ware fallback to `default` when the ware has no variant. Use `v_build_method` / `frames.build_methods`; `station_modules.method` already carries it. Never assume `default` — this playthrough's player faction is `terran`.
