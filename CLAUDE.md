...
- Market tab data semantics (all reverse-engineered, validated against this save):
  ...
  - Build demand = `<insufficient>`/`<shortage>` wares under `<build><resources>`. New-station constructions sit on **free-floating build storages with no station ancestor** — don't require an object ancestor when collecting (this bug once zeroed claytronics demand).
